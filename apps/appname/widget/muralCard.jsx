...
const Selected = styled.span`
  background: linear-gradient(120deg, #ffd50d 0%, #f29bc0 25%, #4f46e5 50%, #f29bc0 75%, #ffd50d 100%);
  color: #000;
...
const MAP_TOKEN = 'pk.eyJ1IjoidGVqMDEiLCJhIjoiY2xqcHZ2dGpkMDB5azNsbzQ0bmMwNjRjaCJ9.FVv2zRPaLwzZMgagbI2YZw'
...
const MagicDot = styled.div`
  ...
  box-shadow: rgba(0, 0, 0, 0.1) 0px 4px 6px, rgba(0, 0, 0, 0.06) 0px 2px 4px, rgba(0, 0, 0, 0.1) 0px 10px 15px;
...
const onMagicDotClick = () => {
  const selected = dataMap.onMarket[Math.floor(Math.random() * dataMap.onMarket.length)]
...
const handleSave = (data) => {
  ...
  Social.set(
    ...
    },
    {
      ...
      onCancel: () => State.update({ edit: false, showForm: false, showInspect: false }),
    }
  )
}

function VerificationIcon() {
  return <img src="https://i.ibb.co/sHQcy2B/icons8-verified-96.png" width="18" height="18" />
}
...
function RatingIcon({ value, max }) {
  ...
  return (
    <span>
      {value}
      {[...Array(fill)].map(() => (
        <img src="https://i.ibb.co/vcZ4HT1/icons8-star-96.png" width="18" height="18" />
      ))}
      {[...Array(unfill)].map(() => (
        <img src="https://i.ibb.co/F8RZxTp/icons8-star-96-empty.png" width="18" height="18" />
      ))}
    </span>
  )
}
...
function Form() {
  return (
    <FormCard>
      ...
      <br />
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <input style={{ width: '45%' }} type="submit" value="Submit" onClick={() => handleSave(State.data)} />
        <input
          style={{ width: '45%' }}
          type="button"
          value="Cancel"
          onClick={() => State.update({ edit: false, showForm: false, data: {} })}
        />
      </div>
    </FormCard>
  )
...
function Inspect() {
  return (
    <InspectCard>
      {/* <img src={state.focusedMarker.imgSrc} /> */}
      {state.focusedMarker.address}
      <ImageWrapper>
        <img
          class="img-fluid"
          src="https://images.unsplash.com/photo-1624198376649-c121a452d157?auto=format&fit=crop&q=80&w=2940&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
        />
        {/* <img src={dataMural.imgSrc} /> */}
      </ImageWrapper>
      <BuildingInfo>
        {state.focusedMarker.stories} Stories | {state.focusedMarker.built} Built
      </BuildingInfo>
      <div> date </div>
      <CardButton>Collect badge</CardButton>
      <Units>
        {state.focusedMarker.units.map((unit) => (
          <Unit>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <div style={{ color: '#9333ea' }}>{unit.onMarket ? `$${unit.price}` : ''}</div>
            </div>
            <div>
